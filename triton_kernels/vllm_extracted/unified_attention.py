...
# ---------------------------------------------------------------------------
# LOCAL EXTRACTION NOTE (see NOTICE.md in this directory).
#
# This file is a verbatim copy of vLLM's
# vllm/v1/attention/ops/triton_unified_attention.py with ONLY the module-
# level imports replaced so the file is importable outside vLLM's package
# tree. All kernel bodies (kernel_unified_attention_2d / _3d, reduce_segments,
# _prepare_kv_tile, find_seq_idx, apply_softcap, cdiv_fn) and the entire
# unified_attention() dispatch function are byte-identical to upstream.
#
# The is_batch_invariant branch is retained as a dead branch guarded by a
# constant False so the line-level dispatch logic below (~line 1157-1166)
# stays identical to upstream for diff purposes.
# ---------------------------------------------------------------------------
...
# Upstream dependency: vllm.v1.kv_cache_interface.KVQuantMode
# Retained here as a local copy with matching integer values so that the
# kernels' `KV_QUANT_MODE: tl.constexpr` comparisons (0/1/2/3) are unchanged.
...
# Upstream: envs.VLLM_BATCH_INVARIANT. We never set it; dispatch always
# follows the normal heuristic path. Retained as a named constant so the
# `or is_batch_invariant` clause in unified_attention() is preserved.
is_batch_invariant = False

# Upstream: torch.finfo(current_platform.fp8_dtype()). L4 / H100 / A100 all
# use the E4M3 fp8 variant; fix it here. If we later benchmark on hardware
# that prefers E5M2 we'd revisit, but that's outside this repo's scope.
