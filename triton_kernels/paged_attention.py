"""
Lesson 11 · Phase 1 — Paged Attention (decode, MHA) in Triton.
...
Scope of Phase 1:
    ...
    - MHA (H_q == H_kv). GQA deferred to Phase 2.
    - fp16 or fp32, head_dim ∈ {64, 128}.
...
block_table[b, i] → phys_blk, load K_cache[phys_blk, :, h, :] and
V_cache[phys_blk, :, h, :], mask against context_lens[b], and fold into
the online-softmax accumulator.
...
def paged_attention_decode_kernel(
    ...
    HEAD_DIM:   tl.constexpr,   # 64 or 128
):
    ...
    for logical_blk in range(0, num_blocks):
        ...
        # 3) Load K block — shape (BLOCK_SIZE, HEAD_DIM)
        k_base = K_cache_ptr + phys_blk * stride_kb + pid_h * stride_kh
        k_ptrs = (k_base
        ...
        # 4) Load V block
        v_base = V_cache_ptr + phys_blk * stride_vb + pid_h * stride_vh
        v_ptrs = (v_base
...
def triton_paged_attention_decode(
    ...
    """Paged attention forward (decode, MHA).

    Args:
        Q: (B, H, d) — last axis contiguous.
        K_cache, V_cache: (num_blocks, block_size, H, d) — last axis contig.
        block_table: (B, max_blocks_per_seq) int32.
        context_lens: (B,) int32.
    ...
    assert H == H_kv, f"Phase 1 is MHA only: H={H} H_kv={H_kv}"
    assert d == d_kv
