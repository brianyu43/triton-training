"""
Lesson 11 · Phase 1 correctness — Triton paged attention vs paged reference.

Requires CUDA + Triton. Runs on the GCP L4 spot VM (or any sm_89).

For each test shape:
    1. Generate random contiguous (B, H, N, d) K/V + (B, H, d) Q.
    2. Pack K/V into paged layout via pack_kv_paged().
    3. Run:
         a) naive_decode_attention (contiguous, ground truth)
         b) paged_attention_ref    (CPU/GPU paged, fp32 math)
         c) triton_paged_attention_decode (our Triton kernel)
    4. Verify (c) matches (a) and (b) within fp16 tolerance.
...
def main():
    ...
    for dtype in (torch.float16, torch.float32):
        ...
        for i, case in enumerate(CASES):
            ...
            print(
                f"[{status}] case {i:2d}: B={r['case']['B']:2d} H={r['case']['H']:2d} "
