...
MODE="${1:-benchmark}"
SUBMISSION="${2:-gpumode/sort_v2/submissions/v00_torch_sort.py}"
CASES="${3:-}"
...
if [[ -z "$CASES" ]]; then
  if [[ "$MODE" == "test" ]]; then
    CASES="gpumode/sort_v2/cases/test_cases.txt"
  else
    CASES="gpumode/sort_v2/cases/benchmark_cases.txt"
  fi
fi
...
LOG_DIR="$ROOT_DIR/gpumode/sort_v2/logs"
LOCAL_LOG="$LOG_DIR/${SUB_BASENAME}_${MODE}_${STAMP}.out"

REMOTE_ROOT="/home/xavier"
REMOTE_REPO="$REMOTE_ROOT/reference-kernels"
REMOTE_PROJ="$REMOTE_ROOT/cudatraining/gpumode/sort_v2"
REMOTE_PROBLEM="$REMOTE_REPO/problems/pmpp_v2/sort_py"
...
gcloud compute scp "$ROOT_DIR/gpumode/sort_v2/official/task.py" "$INSTANCE:$REMOTE_TASK" --zone "$ZONE"
