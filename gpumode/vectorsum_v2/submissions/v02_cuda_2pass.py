...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("vectorsum_forward", &vectorsum_forward, "A100 vectorsum 2-pass forward");
}
"""
...
__device__ __forceinline__ float block_sum(float value) {
  ...
  value = (threadIdx.x < (kBlockSize >> 5)) ? warp_partials[lane] : 0.0f;
  if (warp == 0) {
    value = warp_sum(value);
  }
  return value;
...
int choose_blocks(int64_t n) {
  int device = 0;
  cudaDeviceProp prop;
  C10_CUDA_CHECK(cudaGetDevice(&device));
  C10_CUDA_CHECK(cudaGetDeviceProperties(&prop, device));

  const int max_input_blocks = static_cast<int>((n + kBlockSize - 1) / kBlockSize);
  const int target_blocks = prop.multiProcessorCount * kBlocksPerSm;
  int blocks = target_blocks < max_input_blocks ? target_blocks : max_input_blocks;
...
torch::Tensor vectorsum_forward(torch::Tensor x, torch::Tensor output) {
  ...
  const int blocks = choose_blocks(n);
  auto partials = torch::empty({blocks}, x.options());
  cudaStream_t stream = at::cuda::getCurrentCUDAStream();

  partial_sum_kernel<<<blocks, kBlockSize, 0, stream>>>(
      x.data_ptr<float>(),
      partials.data_ptr<float>(),
      n);
  C10_CUDA_KERNEL_LAUNCH_CHECK();

  final_sum_kernel<<<1, kBlockSize, 0, stream>>>(
      partials.data_ptr<float>(),
...
"""
    ...
    _EXT = load_inline(
        name="vectorsum_v2_a100_2pass_ext",
        cpp_sources=[cpp_src],
