...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("vectorsum_forward", &vectorsum_forward, "A100 vectorsum single-kernel atomic vec4 bps12 forward");
}
"""
...
__global__ void atomic_sum_vec4_single_kernel(
...
torch::Tensor vectorsum_forward(torch::Tensor x, torch::Tensor output) {
  ...
  atomic_sum_vec4_single_kernel<<<g_blocks, kBlockSize, 0, stream>>>(
      x.data_ptr<float>(),
...
"""
    ...
    _EXT = load_inline(
        name="vectorsum_v2_a100_atomic_single_vec4_bps12_ext",
        cpp_sources=[cpp_src],
