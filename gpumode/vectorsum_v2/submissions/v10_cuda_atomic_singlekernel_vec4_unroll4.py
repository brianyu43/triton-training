...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("vectorsum_forward", &vectorsum_forward, "A100 vectorsum atomic vec4 unroll4 forward");
}
"""
...
__global__ void atomic_sum_vec4_unroll4_kernel(
  ...
  const int64_t stride4 = stride * 4;
  for (; idx + 3 * stride < n4; idx += stride4) {
    s0 += sum4(x4[idx]);
    s1 += sum4(x4[idx + stride]);
    s2 += sum4(x4[idx + 2 * stride]);
    s3 += sum4(x4[idx + 3 * stride]);
  }
...
torch::Tensor vectorsum_forward(torch::Tensor x, torch::Tensor output) {
  ...
  atomic_sum_vec4_unroll4_kernel<<<g_blocks, kBlockSize, 0, stream>>>(
      x.data_ptr<float>(),
...
"""
    ...
    _EXT = load_inline(
        name="vectorsum_v2_a100_atomic_vec4_unroll4_ext",
        cpp_sources=[cpp_src],
