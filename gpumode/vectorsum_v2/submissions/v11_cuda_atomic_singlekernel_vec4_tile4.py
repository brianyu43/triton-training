...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("vectorsum_forward", &vectorsum_forward, "A100 vectorsum atomic vec4 tile4 forward");
}
"""
...
__global__ void atomic_sum_vec4_tile4_kernel(
...
torch::Tensor vectorsum_forward(torch::Tensor x, torch::Tensor output) {
  ...
  atomic_sum_vec4_tile4_kernel<<<g_blocks, kBlockSize, 0, stream>>>(
      x.data_ptr<float>(),
...
"""
    ...
    _EXT = load_inline(
        name="vectorsum_v2_a100_atomic_vec4_tile4_ext",
        cpp_sources=[cpp_src],
