...
torch::Tensor trimul_v27_forward(
    torch::Tensor x,
    torch::Tensor mask,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("trimul_v27_forward", &trimul_v27_forward, "trimul v27 cuda stage timing from v26");
}
"""
...
torch::Tensor trimul_v27_forward(
  ...
  if (timing) {
    ...
    std::printf(
        "trimul_stage B=%lld N=%lld C=%lld H=%lld rows=%lld workspace_hit=%d packed_hit=%d "
...
"""
    ...
    _EXT = load_inline(
        name="trimul_v27_cuda_ext_stage_timing_v26",
        cpp_sources=cpp_src,
