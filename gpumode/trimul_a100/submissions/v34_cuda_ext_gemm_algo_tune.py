...
torch::Tensor trimul_v34_forward(
    torch::Tensor x,
    torch::Tensor mask,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("trimul_v34_forward", &trimul_v34_forward, "trimul v34 cuda gemm algo tune");
}
"""
...
static cublasGemmAlgo_t parse_algo_env(const char* name, cublasGemmAlgo_t fallback) {
  const char* value = std::getenv(name);
  if (value == nullptr || value[0] == '\0') {
    return fallback;
  }
  if (std::strcmp(value, "0") == 0 ||
      std::strcmp(value, "algo0") == 0 ||
      std::strcmp(value, "ALGO0") == 0) {
    return CUBLAS_GEMM_ALGO0_TENSOR_OP;
  }
  if (std::strcmp(value, "1") == 0 ||
      std::strcmp(value, "algo1") == 0 ||
      std::strcmp(value, "ALGO1") == 0) {
...
static cublasGemmAlgo_t central_static_algo(
    int64_t bs,
    int64_t n,
    int64_t dim,
    int64_t hidden_dim) {
  if (hidden_dim == 128 && bs == 1 && n == 1024 && dim <= 384) {
    return CUBLAS_GEMM_ALGO1_TENSOR_OP;
  }
  return CUBLAS_GEMM_DEFAULT_TENSOR_OP;
}
...
torch::Tensor trimul_v34_forward(
  ...
  const cublasGemmAlgo_t proj_algo =
      parse_algo_env("TRIMUL_PROJ_ALGO", CUBLAS_GEMM_DEFAULT_TENSOR_OP);
  const cublasGemmAlgo_t central_algo =
      parse_algo_env("TRIMUL_CENTRAL_ALGO", central_static_algo(bs, n, dim, hidden_dim));
  const cublasGemmAlgo_t out_algo =
      parse_algo_env("TRIMUL_OUT_ALGO", CUBLAS_GEMM_DEFAULT_TENSOR_OP);
...
"""
    ...
    _EXT = load_inline(
        name="trimul_v34_cuda_ext_gemm_algo_tune",
        cpp_sources=cpp_src,
