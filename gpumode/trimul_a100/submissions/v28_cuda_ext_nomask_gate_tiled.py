...
torch::Tensor trimul_v28_forward(
    torch::Tensor x,
    torch::Tensor mask,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("trimul_v28_forward", &trimul_v28_forward, "trimul v28 cuda nomask gate tiled");
}
"""
...
__global__ void apply_gate_nomask_kernel(
    __half* __restrict__ lr5,
    int64_t rows,
    int hidden_dim) {
  const int64_t idx =
  ...
  const float left_gate = __half2float(lr5[left_gate_idx]);
  const float right_gate = __half2float(lr5[right_gate_idx]);
  lr5[left_idx] = __float2half_rn(left * sigmoid_f(left_gate));
  lr5[right_idx] = __float2half_rn(right * sigmoid_f(right_gate));
}
...
torch::Tensor trimul_v28_forward(
  ...
  if (mask.scalar_type() == torch::kFloat32) {
    apply_gate_nomask_kernel<<<gate_blocks, gate_threads>>>(
        reinterpret_cast<__half*>(lr5.data_ptr<at::Half>()),
        rows,
  ...
  constexpr int kHiddenWarpsPerBlock = 8;
  const unsigned int hidden_warp_blocks =
      static_cast<unsigned int>((rows + kHiddenWarpsPerBlock - 1) / kHiddenWarpsPerBlock);
  if (hidden_dim == 128) {
    const size_t hidden_shmem = 2 * kHiddenWarpsPerBlock * 128 * sizeof(__half);
    hidden_ln_gate_to_rows_tiled_kernel<128, kHiddenWarpsPerBlock><<<hidden_warp_blocks, ln_threads, hidden_shmem>>>(
...
"""
    ...
    _EXT = load_inline(
        name="trimul_v28_cuda_ext_nomask_gate_tiled",
        cpp_sources=cpp_src,
