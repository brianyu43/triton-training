...
def _get_ext() -> Any:
    ...
    ext_name = f"trimul_a100_f16_v39_stage_cap{cache_cap}"
...
torch::Tensor trimul_forward(
  ...
  if (stage_timing.enabled && read_bool_env("TRIMUL_STAGE_EACH_CALL")) {
  ...
  cudaEvent_t ev_total_begin = nullptr;
  cudaEvent_t ev_total_end = nullptr;
  ...
  cudaEvent_t ev_hidden_begin = nullptr;
  cudaEvent_t ev_hidden_end = nullptr;
  cudaEvent_t ev_final_begin = nullptr;
  cudaEvent_t ev_final_end = nullptr;
  if (profile_this_call) {
    stage_timing.profile_epoch += 1;
    cudaEventCreateWithFlags(&ev_total_begin, cudaEventDefault);
    cudaEventCreateWithFlags(&ev_total_end, cudaEventDefault);
    ...
    cudaEventCreateWithFlags(&ev_hidden_begin, cudaEventDefault);
    cudaEventCreateWithFlags(&ev_hidden_end, cudaEventDefault);
    cudaEventCreateWithFlags(&ev_final_begin, cudaEventDefault);
    cudaEventCreateWithFlags(&ev_final_end, cudaEventDefault);
    if (ev_total_begin != nullptr) cudaEventRecord(ev_total_begin, 0);
  ...
  if (profile_this_call) {
    ...
    if (ev_hidden_begin != nullptr) cudaEventRecord(ev_hidden_begin, 0);
  }
  ...
  if (profile_this_call) {
    if (ev_hidden_end != nullptr) cudaEventRecord(ev_hidden_end, 0);
  }
  ...
  if (profile_this_call) {
    ...
    if (ev_total_end != nullptr) cudaEventRecord(ev_total_end, 0);
    if (ev_total_end != nullptr) {
      cudaEventSynchronize(ev_total_end);
    } else if (ev_final_end != nullptr) {
      cudaEventSynchronize(ev_final_end);
    }
    ...
    if (ev_hidden_begin != nullptr && ev_hidden_end != nullptr)
      cudaEventElapsedTime(&hidden_ms, ev_hidden_begin, ev_hidden_end);
    if (ev_final_begin != nullptr && ev_final_end != nullptr)
      cudaEventElapsedTime(&final_ms, ev_final_begin, ev_final_end);
    if (ev_total_begin != nullptr && ev_total_end != nullptr)
      cudaEventElapsedTime(&total_ms, ev_total_begin, ev_total_end);

    std::printf(
        "trimul_rank02_stage_v39 B=%lld N=%lld C=%lld H=%lld rows=%lld mask_dtype=%d ln_ms=%.4f proj_gate_ms=%.4f central_ms=%.4f hidden_ms=%.4f out_ms=%.4f total_ms=%.4f\n",
    ...
    if (ev_total_begin != nullptr) cudaEventDestroy(ev_total_begin);
    if (ev_total_end != nullptr) cudaEventDestroy(ev_total_end);
    ...
    if (ev_hidden_begin != nullptr) cudaEventDestroy(ev_hidden_begin);
    if (ev_hidden_end != nullptr) cudaEventDestroy(ev_hidden_end);
