...
torch::Tensor trimul_v40_forward(
  ...
  if (dim == 128) {
    layernorm_warp_rows_to_f16_kernel<128, kLnWarpsPerBlock><<<warp_ln_blocks, ln_threads, 0, s>>>(
        x.data_ptr<float>(),
    ...
    layernorm_warp_rows_to_f16_kernel<384, kLnWarpsPerBlock><<<warp_ln_blocks, ln_threads, 0, s>>>(
        x.data_ptr<float>(),
        norm_weight.data_ptr<float>(),
        norm_bias.data_ptr<float>(),
        reinterpret_cast<__half*>(xhat.data_ptr<at::Half>()),
    ...
    layernorm_warp_rows_to_f16_kernel<768, kLnWarpsPerBlock><<<warp_ln_blocks, ln_threads, 0, s>>>(
        x.data_ptr<float>(),
        norm_weight.data_ptr<float>(),
        norm_bias.data_ptr<float>(),
        reinterpret_cast<__half*>(xhat.data_ptr<at::Half>()),
    ...
    layernorm_rows_to_f16_kernel<<<static_cast<unsigned int>(rows), ln_threads, shmem, s>>>(
        x.data_ptr<float>(),
        norm_weight.data_ptr<float>(),
        norm_bias.data_ptr<float>(),
        reinterpret_cast<__half*>(xhat.data_ptr<at::Half>()),
  ...
  const bool force_repack = dim == 384 && n == 1024 && mask.scalar_type() != torch::kFloat32;
  ...
  if (!packed_hit) {
    ...
    pack6_f32_to_f16_kernel<<<pack_blocks, pack_threads, 0, s>>>(
  ...
  cublas_check(cublasSetStream(handle, s), "cublasSetStream failed");
  ...
  if (use_old_gate == 0 && mask.scalar_type() == torch::kFloat32) {
    apply_gate_nomask_vec4_kernel<kVecGateThreads><<<vec_gate_blocks, kVecGateThreads, 0, s>>>(
        reinterpret_cast<__half*>(lr5.data_ptr<at::Half>()),
        static_cast<int>(rows),
        static_cast<int>(hidden_dim));
  } else if (use_old_gate == 0) {
    apply_gate_mask_vec4_kernel<kVecGateThreads, int64_t><<<vec_gate_blocks, kVecGateThreads, 0, s>>>(
        reinterpret_cast<__half*>(lr5.data_ptr<at::Half>()),
        mask.data_ptr<int64_t>(),
        static_cast<int>(rows),
        static_cast<int>(hidden_dim));
  } else if (mask.scalar_type() == torch::kFloat32) {
    apply_gate_mask_kernel<float><<<gate_blocks, gate_threads, 0, s>>>(
        reinterpret_cast<__half*>(lr5.data_ptr<at::Half>()),
        mask.data_ptr<float>(),
        rows,
        static_cast<int>(hidden_dim));
  } else {
    apply_gate_mask_kernel<int64_t><<<gate_blocks, gate_threads, 0, s>>>(
        reinterpret_cast<__half*>(lr5.data_ptr<at::Half>()),
        mask.data_ptr<int64_t>(),
        rows,
        static_cast<int>(hidden_dim));
  ...
  if (hidden_dim == 128 && use_old_hidden == 0) {
    ...
    hidden_ln_gate_to_rows_rank02_kernel<128><<<hidden_grid, hidden_block, 0, s>>>(
    ...
    if (n >= 768) {
      ...
      hidden_ln_gate_to_rows_tiled_kernel<128, kHiddenTileRows><<<hidden_blocks, kHiddenThreads, hidden_shmem, s>>>(
          reinterpret_cast<const __half*>(central.data_ptr<at::Half>()),
          out_gate_ptr,
          to_out_norm_weight.data_ptr<float>(),
          to_out_norm_bias.data_ptr<float>(),
    ...
    hidden_ln_gate_to_rows_kernel<<<static_cast<unsigned int>(rows), ln_threads, shmem, s>>>(
        reinterpret_cast<const __half*>(central.data_ptr<at::Half>()),
        out_gate_ptr,
        to_out_norm_weight.data_ptr<float>(),
        to_out_norm_bias.data_ptr<float>(),
...
torch::Tensor trimul_v48_tail(
  ...
  if (!packed_hit) {
    ...
    pack1_f32_to_f16_kernel<<<pack_blocks, pack_threads, 0, s>>>(
        to_out_weight.data_ptr<float>(),
...
torch::Tensor trimul_v50_pack_rank01_weights(
  ...
  auto s = at::cuda::getCurrentCUDAStream();
  pack6_rank01_transpose_f32_to_f16_kernel<<<blocks, threads, 0, s>>>(
      left_proj_weight.data_ptr<float>(),
      right_proj_weight.data_ptr<float>(),
      left_gate_weight.data_ptr<float>(),
      right_gate_weight.data_ptr<float>(),
...
torch::Tensor trimul_v51_pack5_transpose_weights(
  ...
  auto s = at::cuda::getCurrentCUDAStream();
  pack5_transpose_f32_to_f16_kernel<<<blocks, threads, 0, s>>>(
      left_proj_weight.data_ptr<float>(),
      right_proj_weight.data_ptr<float>(),
      left_gate_weight.data_ptr<float>(),
      right_gate_weight.data_ptr<float>(),
