...
_RANK01_WEIGHT_CACHE: Dict[Any, Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]] = {}
...
torch::Tensor trimul_v48_tail(
  ...
  if (timing) {
    ...
    float out_ms = 0.0f;
    float total_ms = 0.0f;
    cuda_check(cudaEventElapsedTime(&central_ms, ev_start, ev_central), "elapsed central failed");
    cuda_check(cudaEventElapsedTime(&hidden_ms, ev_central, ev_hidden), "elapsed hidden failed");
    cuda_check(cudaEventElapsedTime(&out_ms, ev_hidden, ev_out), "elapsed out failed");
    cuda_check(cudaEventElapsedTime(&total_ms, ev_start, ev_out), "elapsed total failed");
    std::printf(
        "trimul_tail_v48 B=%lld N=%lld C=%lld H=%lld rows=%lld packed_hit=%d "
        "central_ms=%.4f hidden_ms=%.4f out_ms=%.4f total_ms=%.4f\n",
        static_cast<long long>(bs),
        static_cast<long long>(n),
        static_cast<long long>(dim),
        static_cast<long long>(hidden_dim),
...
"""
    ...
    _EXT = load_inline(
        name="trimul_v49_v48_rank01_weight_cache",
        cpp_sources=cpp_src,
...
def _rank01_weights_fp16(
    ...
    if os.environ.get("TRIMUL_V49_DISABLE_RANK01_WEIGHT_CACHE", "") == "1":
        return (
            weights["left_proj.weight"].t().contiguous().to(torch.float16),
            weights["right_proj.weight"].t().contiguous().to(torch.float16),
            weights["left_gate.weight"].t().contiguous().to(torch.float16),
            weights["right_gate.weight"].t().contiguous().to(torch.float16),
            weights["out_gate.weight"].t().contiguous().to(torch.float16),
            weights["to_out.weight"].t().contiguous().to(torch.float16),
        )
...
def _v48_c384_path(
    ...
    if enable_timing:
        ...
        ev_tail = torch.cuda.Event(enable_timing=True)
        ev_start.record()
    else:
        ev_start = ev_ln = ev_prep = ev_proj = ev_tail = None
    ...
    lr5 = torch.empty((5 * hidden_dim, B, N, N), dtype=torch.float16, device=device)
    if enable_timing:
        ev_prep.record()
    ...
    if enable_timing:
        ...
        tail_ms = ev_proj.elapsed_time(ev_tail)
        total_ms = ev_start.elapsed_time(ev_tail)
        print(
            "trimul_stage_v48 "
            f"path=c384_proj_gate_tail B={B} N={N} C={dim} H={hidden_dim} rows={M} "
            f"nomask={1 if nomask else 0} ln_ms={ln_ms:.4f} prep_ms={prep_ms:.4f} "
            f"proj_gate_ms={proj_gate_ms:.4f} tail_ms={tail_ms:.4f} total_ms={total_ms:.4f}",
            flush=True,
        )
