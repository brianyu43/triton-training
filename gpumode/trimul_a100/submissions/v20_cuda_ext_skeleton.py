...
torch::Tensor trimul_v20_forward(
    torch::Tensor x,
    torch::Tensor mask,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("trimul_v20_forward", &trimul_v20_forward, "trimul v20 cuda skeleton");
}
"""
...
torch::Tensor trimul_v20_forward(
  ...
  auto xhat = torch::empty({rows, dim}, opts_f16);
  auto packed = torch::empty({6 * hidden_dim * dim}, opts_f16);
  auto lr5 = torch::empty({5 * hidden_dim, bs, n, n}, opts_f16);
  auto central = torch::empty({hidden_dim, bs, n, n}, opts_f16);
  auto hidden_rows = torch::empty({rows, hidden_dim}, opts_f16);
  ...
  const __half* packed_ptr = reinterpret_cast<const __half*>(packed.data_ptr<at::Half>());
  gemm_f16_abt(
      handle,
      packed_ptr,
...
"""
    ...
    _EXT = load_inline(
        name="trimul_v20b_cuda_ext_skeleton",
        cpp_sources=cpp_src,
