...
_USE_OLD_GATE = os.environ.get("TRIMUL_V37_OLD_GATE", "") == "1"
...
torch::Tensor trimul_v37_forward(
    torch::Tensor x,
    torch::Tensor mask,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("trimul_v37_forward", &trimul_v37_forward, "trimul v37 cuda vectorized gate");
}
"""
...
torch::Tensor trimul_v37_forward(
  ...
  if (timing) {
    ...
    std::printf(
        "trimul_stage_v37 B=%lld N=%lld C=%lld H=%lld rows=%lld workspace_hit=%d packed_hit=%d "
...
"""
    ...
    _EXT = load_inline(
        name="trimul_v37_cuda_ext_vec_gate",
        cpp_sources=cpp_src,
