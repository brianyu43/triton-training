...
def _fallback_functional(
    ...
    left = F.linear(x_norm, weights["left_proj.weight"])
...
torch::Tensor trimul_v40_forward(
  ...
  const bool force_repack = dim == 384 && n == 1024;
...
def custom_kernel(data: Tuple[torch.Tensor, torch.Tensor, Dict[str, torch.Tensor], Dict[str, Any]]) -> torch.Tensor:
    ...
    if hidden_dim == 128 and C == 384 and B == 1 and N == 1024:
        if cfg_nomask is None:
            is_nomask = mask is None or bool(torch.amin(mask).item() == 1)
        if not is_nomask:
            return _fallback_functional(x, mask, weights, C, hidden_dim)
