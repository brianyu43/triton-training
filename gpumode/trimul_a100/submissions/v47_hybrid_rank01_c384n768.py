...
# Rank01 Triton path used for measured winner shapes. v42 proved the large/mid
# C128 rows are worth routing here; v47 probes the single C384 row where the
# same-session v46 table showed rank01 slightly ahead of v40.
# ----------------------------------------------------------------------
...
def custom_kernel(data: Tuple[torch.Tensor, torch.Tensor, Dict[str, torch.Tensor], Dict[str, Any]]) -> torch.Tensor:
    ...
    use_rank01_c128 = C == 128 and B == 1 and N in (512, 768, 1024)
    use_rank01_c384 = C == 384 and B == 1 and N == 768
    if hidden_dim == 128 and (use_rank01_c128 or use_rank01_c384):
        return _rank01_path(data)
