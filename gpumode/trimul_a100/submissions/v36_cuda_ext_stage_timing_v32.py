...
torch::Tensor trimul_v36_forward(
    torch::Tensor x,
    torch::Tensor mask,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("trimul_v36_forward", &trimul_v36_forward, "trimul v36 cuda stage timing from v32");
}
"""
...
torch::Tensor trimul_v36_forward(
  ...
  if (timing) {
    ...
    std::printf(
        "trimul_stage_v36 B=%lld N=%lld C=%lld H=%lld rows=%lld workspace_hit=%d packed_hit=%d "
...
"""
    ...
    _EXT = load_inline(
        name="trimul_v36_cuda_ext_stage_timing_v32",
        cpp_sources=cpp_src,
