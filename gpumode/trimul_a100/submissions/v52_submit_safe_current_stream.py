...
torch::Tensor trimul_v40_forward(
  ...
  if (dim == 128) {
    layernorm_warp_rows_to_f16_kernel<128, kLnWarpsPerBlock><<<warp_ln_blocks, ln_threads, 0, stream>>>(
        x.data_ptr<float>(),
    ...
    layernorm_warp_rows_to_f16_kernel<384, kLnWarpsPerBlock><<<warp_ln_blocks, ln_threads, 0, stream>>>(
        x.data_ptr<float>(),
        norm_weight.data_ptr<float>(),
        norm_bias.data_ptr<float>(),
        reinterpret_cast<__half*>(xhat.data_ptr<at::Half>()),
    ...
    layernorm_warp_rows_to_f16_kernel<768, kLnWarpsPerBlock><<<warp_ln_blocks, ln_threads, 0, stream>>>(
        x.data_ptr<float>(),
        norm_weight.data_ptr<float>(),
        norm_bias.data_ptr<float>(),
        reinterpret_cast<__half*>(xhat.data_ptr<at::Half>()),
    ...
    layernorm_rows_to_f16_kernel<<<static_cast<unsigned int>(rows), ln_threads, shmem, stream>>>(
        x.data_ptr<float>(),
        norm_weight.data_ptr<float>(),
        norm_bias.data_ptr<float>(),
        reinterpret_cast<__half*>(xhat.data_ptr<at::Half>()),
  ...
  if (!packed_hit) {
    ...
    pack6_f32_to_f16_kernel<<<pack_blocks, pack_threads, 0, stream>>>(
  ...
  cublas_check(cublasSetStream(handle, stream), "cublasSetStream failed");
  ...
  if (use_old_gate == 0 && mask.scalar_type() == torch::kFloat32) {
    apply_gate_nomask_vec4_kernel<kVecGateThreads><<<vec_gate_blocks, kVecGateThreads, 0, stream>>>(
        reinterpret_cast<__half*>(lr5.data_ptr<at::Half>()),
        static_cast<int>(rows),
        static_cast<int>(hidden_dim));
  } else if (use_old_gate == 0) {
    apply_gate_mask_vec4_kernel<kVecGateThreads, int64_t><<<vec_gate_blocks, kVecGateThreads, 0, stream>>>(
        reinterpret_cast<__half*>(lr5.data_ptr<at::Half>()),
        mask.data_ptr<int64_t>(),
        static_cast<int>(rows),
        static_cast<int>(hidden_dim));
  } else if (mask.scalar_type() == torch::kFloat32) {
    apply_gate_mask_kernel<float><<<gate_blocks, gate_threads, 0, stream>>>(
        reinterpret_cast<__half*>(lr5.data_ptr<at::Half>()),
        mask.data_ptr<float>(),
        rows,
        static_cast<int>(hidden_dim));
  } else {
    apply_gate_mask_kernel<int64_t><<<gate_blocks, gate_threads, 0, stream>>>(
        reinterpret_cast<__half*>(lr5.data_ptr<at::Half>()),
        mask.data_ptr<int64_t>(),
        rows,
        static_cast<int>(hidden_dim));
  ...
  if (hidden_dim == 128 && use_old_hidden == 0) {
    ...
    hidden_ln_gate_to_rows_rank02_kernel<128><<<hidden_grid, hidden_block, 0, stream>>>(
    ...
    if (n >= 768) {
      ...
      hidden_ln_gate_to_rows_tiled_kernel<128, kHiddenTileRows><<<hidden_blocks, kHiddenThreads, hidden_shmem, stream>>>(
          reinterpret_cast<const __half*>(central.data_ptr<at::Half>()),
          out_gate_ptr,
          to_out_norm_weight.data_ptr<float>(),
          to_out_norm_bias.data_ptr<float>(),
    ...
    hidden_ln_gate_to_rows_kernel<<<static_cast<unsigned int>(rows), ln_threads, shmem, stream>>>(
        reinterpret_cast<const __half*>(central.data_ptr<at::Half>()),
        out_gate_ptr,
        to_out_norm_weight.data_ptr<float>(),
        to_out_norm_bias.data_ptr<float>(),
...
torch::Tensor trimul_v48_tail(
  ...
  at::cuda::CUDAGuard device_guard(lr5.device());
  ...
  PackedSingleWeightCache& out_cache = caches.to_out_weight;
  const void* p = to_out_weight.data_ptr<float>();
  const int64_t v = tensor_version(to_out_weight);
  const bool packed_hit =
      out_cache.packed.defined() &&
      out_cache.elems == out_elems &&
      out_cache.p == p &&
      out_cache.v == v;
  if (!packed_hit) {
    out_cache.packed = torch::empty({out_elems}, opts_f16);
    const int pack_threads = 256;
    const int pack_blocks = static_cast<int>((out_elems + pack_threads - 1) / pack_threads);
    pack1_f32_to_f16_kernel<<<pack_blocks, pack_threads, 0, stream>>>(
        to_out_weight.data_ptr<float>(),
        reinterpret_cast<__half*>(out_cache.packed.data_ptr<at::Half>()),
        out_elems);
    out_cache.p = p;
    out_cache.v = v;
    out_cache.elems = out_elems;
  ...
  gemm_f16_abt(
      handle,
      reinterpret_cast<const __half*>(hidden_rows.data_ptr<at::Half>()),
      reinterpret_cast<const __half*>(out_cache.packed.data_ptr<at::Half>()),
      reinterpret_cast<__half*>(y.data_ptr<at::Half>()),
      rows,
      hidden_dim,
      dim);
...
torch::Tensor trimul_v50_pack_rank01_weights(
  ...
  at::cuda::CUDAGuard device_guard(left_proj_weight.device());
  ...
  cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  pack6_rank01_transpose_f32_to_f16_kernel<<<blocks, threads, 0, stream>>>(
      left_proj_weight.data_ptr<float>(),
      right_proj_weight.data_ptr<float>(),
      left_gate_weight.data_ptr<float>(),
      right_gate_weight.data_ptr<float>(),
...
torch::Tensor trimul_v51_pack5_transpose_weights(
  ...
  cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  pack5_transpose_f32_to_f16_kernel<<<blocks, threads, 0, stream>>>(
      left_proj_weight.data_ptr<float>(),
      right_proj_weight.data_ptr<float>(),
      left_gate_weight.data_ptr<float>(),
      right_gate_weight.data_ptr<float>(),
