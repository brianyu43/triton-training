...
if [[ $# -lt 3 ]]; then
  cat <<'EOF'
Usage:
  ./scripts/gcp_run_milestone3_rmsnorm_backward.sh PROJECT_ID ZONE VM_NAME
...
python3 -m pytest -q tests/test_rmsnorm.py
python3 -m benchmarks.bench_rmsnorm_backward --batch 16 --seq 128 --hidden 128 --dtype float16 --warmup 20 --iters 100
EOF
)"
