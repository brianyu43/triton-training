...
if [[ $# -lt 3 ]]; then
  cat <<'EOF'
Usage:
  ./scripts/gcp_run_milestone4_triton_rmsnorm_model.sh PROJECT_ID ZONE VM_NAME [MAX_ITERS]
...
python3 -m pytest -q tests/test_model_ref.py tests/test_rmsnorm.py
rm -rf out/loss_regression_ref out/loss_regression_triton_rmsnorm
python3 -m nanotriton.train --config configs/tiny_shakespeare_ref.yaml --max-iters ${MAX_ITERS} --out-dir out/loss_regression_ref
python3 -m nanotriton.train --config configs/tiny_shakespeare_triton_rmsnorm.yaml --max-iters ${MAX_ITERS} --out-dir out/loss_regression_triton_rmsnorm
python3 scripts/compare_loss_curves.py \
  --ref out/loss_regression_ref/metrics.jsonl \
  --triton out/loss_regression_triton_rmsnorm/metrics.jsonl \
  --max-final-val-diff 0.25
python3 -m nanotriton.generate \
  --ckpt out/loss_regression_triton_rmsnorm/checkpoint.pt \
  --prompt "To be" \
  --max-new-tokens 80
EOF
)"
