...
## 다음 세션 — 남은 MQA gap 을 닫으려면

**Phase 4.5-b (Lesson 12 입구)**: ctx 축 split-k.
...
L4 기준 예상:
- MQA B=16 ctx=4k: 16 programs → ~128 programs. 58 SM 위에서 2.2 wave.
- 두 kernel launch 오버헤드 +15 µs. 짧은 ctx 에서는 regression 이니 auto-dispatch 조건부.

구현 추정: **반나절** → **실제로 반나절에 끝났고, paged 자체 1.68× 가속**. 다만 MQA 의 SDPA gap 이 완전히 닫히진 않음 — 남은 건 L2 residency 라는 다른 story 였음. 상세는 다음 편 [`vLLM 의 Paged Attention V2 를 Triton 으로 — MQA 의 SM 점유율 구멍을 split-k 로 닫기`](/Users/xavier/dev/cudatraining/docs/blog_draft_lesson_12_split_k.md:1).
...
- Lesson 12 (후속편, split-k 로 MQA SM 점유율 구멍 닫기): [`docs/blog_draft_lesson_12_split_k.md`](/Users/xavier/dev/cudatraining/docs/blog_draft_lesson_12_split_k.md:1)
